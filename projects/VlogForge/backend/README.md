...
│       ├── Caching/
│       ├── DependencyInjection.cs
│       └── {VlogForge}.Infrastructure.csproj
├── tests/
│   ├── {VlogForge}.UnitTests/
│   │   ├── Domain/
│   │   ├── Application/
│   │   └── {VlogForge}.UnitTests.csproj
│   ├── {VlogForge}.IntegrationTests/
│   │   ├── Api/
│   │   ├── Fixtures/
│   │   └── {VlogForge}.IntegrationTests.csproj
│   └── {VlogForge}.ArchitectureTests/
│       └── {VlogForge}.ArchitectureTests.csproj
├── docker/
│   ├── Dockerfile
│   └── docker-compose.yml
├── docs/
...
2. **Rename placeholders**:
   ```bash
   # Replace {VlogForge} with your actual project name
   find . -type f -name "*.cs" -exec sed -i '' 's/{VlogForge}/YourProject/g' {} +
   find . -type f -name "*.csproj" -exec sed -i '' 's/{VlogForge}/YourProject/g' {} +
   find . -type f -name "*.sln" -exec sed -i '' 's/{VlogForge}/YourProject/g' {} +
   ```
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Database={VlogForge};Username=postgres;Password="
  },
  "Jwt": {
    "Secret": "",
    "Issuer": "VlogForge",
...
|-----------|----------------|----------|
| Unit Tests | >80% | `tests/{VlogForge}.UnitTests` |
| Integration Tests | Critical paths | `tests/{VlogForge}.IntegrationTests` |
| Architecture Tests | Dependencies | `tests/{VlogForge}.ArchitectureTests` |
