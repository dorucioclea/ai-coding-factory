...
program
  ...
  .alias("acs")
  .description(
    "Synchronize AI coding assistant configurations across Claude, OpenCode, Cursor, Codex, and Windsurf"
  )
  .version("1.0.0")
  .option(
    "-p, --project <path>",
...
program
  ...
  .option(
    "-t, --targets <systems...>",
    "Target systems (default: opencode, cursor, codex)"
  )
  .option(
    "--types <types...>",
    "Artifact types to sync (skill, agent, command, hook, rule)"
  )
...
program
  ...
  .option(
    "--systems <systems...>",
    "Systems to initialize (default: all available)"
  )
...
program
  ...
  .option(
    "-t, --targets <systems...>",
    "Target systems (default: opencode, cursor, codex, windsurf)"
  )
  .option("-n, --dry-run", "Show what would be synced without making changes")
  .option("-v, --verbose", "Show detailed output")
  .action(async (options) => {
...
program
  ...
  .option(
    "-t, --targets <systems...>",
    "Target systems (default: gemini, aider, continue, cody)"
  )
  .option("-n, --dry-run", "Show what would be synced without making changes")
  .option("-v, --verbose", "Show detailed output")
  .action(async (options) => {
...
function resolveProjectRoot(path: string): string {
  ...
  if (!hasIndicator) {
    ...
    console.log(
      chalk.dim(
        `Consider running ${chalk.cyan("acs init")} to initialize the project.\n`
      )
    );
  }
