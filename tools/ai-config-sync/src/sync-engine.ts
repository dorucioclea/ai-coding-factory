...
export class SyncEngine {
  ...
  private async syncArtifact(
    ...
    try {
      ...
      if (!capabilities[typeCapability]) {
        ...
        if ((artifact.type === "skill" || artifact.type === "agent") && capabilities.rules) {
          ...
          return await this.writeArtifact(
            ...
            options,
            baseResult,
            undefined, // no symlink for transformed artifacts
      ...
      return await this.writeArtifact(
        ...
        wasTransformed ? artifact.id : undefined // pass original ID if transformed
      );
    } catch (error) {
      return {
        ...baseResult,
  ...
  async syncMcpServers(options: {
    ...
    try {
      ...
      const servers = await (sourceAdapter as unknown as { readMcpServers: (root: string) => Promise<McpServerArtifact[]> }).readMcpServers(this.projectRoot);
      ...
      for (const target of options.targets) {
        ...
        try {
          ...
          const transformedServers = servers.map((server) => ({
            ...
            sourceSystem: target,
          })) as McpServerArtifact[];

          // Write MCP servers to target
          await (targetAdapter as unknown as { writeMcpServers: (root: string, servers: McpServerArtifact[]) => Promise<void> }).writeMcpServers(this.projectRoot, transformedServers);
  ...
  async generateSkillIndex(options: {
    ...
    try {
      ...
      const skills = artifacts.filter((a): a is SkillArtifact => a.type === "skill");
      ...
      for (const skill of skills) {
        ...
        // Use extracted description if frontmatter description is truncated (ends with ...)
        const frontmatterDesc = skill.description;
        const extractedDesc = this.extractDescription(skill.content);
        const description = (frontmatterDesc && !frontmatterDesc.endsWith("..."))
          ? frontmatterDesc
          : extractedDesc;
      ...
      // Generate a compact skill index - just names and descriptions
      // No file paths since limited systems don't have the skills synced
      let content = `# Skill Index

This is a reference of available skills. To use a skill, ask by name (e.g., "use the debugging skill").
...
`;
      ...
      const allSkills = skills
        .map((s) => ({
          name: s.name,
          description: (s.description && !s.description.endsWith("..."))
            ? s.description
            : this.extractDescription(s.content),
        }))
      ...
      content += `---\n\n*Generated by ai-config-sync • ${skills.length} skills*\n`;
  ...
  private extractDescription(content: string): string {
    ...
    // Get first meaningful paragraph (skip headers, blockquotes, and list items)
    ...
    for (const line of lines) {
      ...
      // Skip empty lines, headers, blockquotes at the start
      if (!foundStart) {
      ...
      // Stop at next header, empty line, or list
      if (!trimmed || trimmed.startsWith("#") || trimmed.startsWith("-") || trimmed.startsWith("*") || trimmed.startsWith(">")) {
        break;
      }
    ...
    // Join and return full description
  ...
  async syncSkillIndex(options: {
    ...
    const limitedSystems: SystemId[] = ["gemini", "aider", "continue", "cody"];
    ...
    spinner.text = `Found ${skills.length} skills, syncing to limited systems...`;

    // Get skill directory path for each system
    const getSkillsDir = (target: SystemId): string => {
      switch (target) {
        case "aider": return ".aider/skills";
        case "continue": return ".continue/skills";
        case "cody": return ".sourcegraph/skills";
        case "gemini": return "skills"; // root level for Gemini
        default: return "skills";
      }
    };

    // Get skill index path for each system
    const getIndexPath = (target: SystemId): string => {
      switch (target) {
        case "aider": return ".aider/SKILL_INDEX.md";
        case "continue": return ".continue/SKILL_INDEX.md";
        case "cody": return ".sourcegraph/SKILL_INDEX.md";
        case "gemini": return "SKILL_INDEX.md";
        default: return "SKILL_INDEX.md";
      }
    };
    ...
    for (const target of targets) {
      ...
      try {
        ...
        const fullSkillsDir = join(this.projectRoot, skillsDir);
        ...
        if (options.dryRun) {
          ...
          results.details.push({
            target,
            status: "success",
            message: `Would copy ${skills.length} skills to ${skillsDir}/ and create index`,
          });
          continue;
        }
        ...
        if (!existsSync(fullSkillsDir)) {
          mkdirSync(fullSkillsDir, { recursive: true });
        }

        // Copy each skill file (strip frontmatter, just the content)
        const skillEntries: Array<{ name: string; description: string; path: string }> = [];

        for (const skill of skills) {
          const skillFileName = `${skill.name}.md`;
          const skillPath = join(skillsDir, skillFileName);
          const fullSkillPath = join(this.projectRoot, skillPath);

          // Read original skill content and strip frontmatter
          const originalPath = join(this.projectRoot, skill.sourcePath);
          let content = "";
          ...
          // Write skill file
          writeFileSync(fullSkillPath, content);

          // Get description for index
          const description = (skill.description && !skill.description.endsWith("..."))
            ? skill.description
            : this.extractDescription(skill.content);
        ...
        // Generate skill index with LOCAL paths
        const indexContent = this.generateSkillIndexContent(skillEntries, target);
        writeFileSync(join(this.projectRoot, indexPath), indexContent);

        results.synced++;
        results.details.push({
          target,
          status: "success",
          message: `Copied ${skills.length} skills to ${skillsDir}/ and created index`,
        });
      } catch (error) {
        results.failed++;
        results.details.push({
    ...
    if (results.synced > 0) {
      spinner.succeed(`Synced ${skills.length} skills to ${results.synced} system(s)`);
    } else if (results.failed > 0) {
      spinner.fail("Failed to sync skills");
    } else {
      spinner.warn("No systems to sync skills to");
    }
...
`;
    ...
    for (const skill of sorted) {
      content += `| \`${skill.name}\` | ${skill.description} | \`${skill.path}\` |\n`;
    }
    ...
    for (const skill of sorted) {
      const category = this.categorizeSkill(skill.name, skill.description);
