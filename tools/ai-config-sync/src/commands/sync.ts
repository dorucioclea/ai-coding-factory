...
import { SyncEngine, type SyncOptions, type SyncSummary } from "../sync-engine.js";
...
function parseTargets(targets?: string[]): SystemId[] {
  if (!targets || targets.length === 0) {
    return ["opencode", "cursor", "codex"] as SystemId[];
  }
...
function printSummary(summary: SyncSummary, verbose?: boolean): void {
  ...
  if (verbose && details.length > 0) {
    ...
    console.log(
      table(
        [["Type", "Name", "Target", "Operation", "Status", "Message"], ...detailData],
        {
          columns: [
            { width: 10 },
            { width: 25 },
