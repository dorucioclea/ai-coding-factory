...
export interface SystemsCommandOptions {
  all?: boolean;
}

export async function systemsCommand(
  projectRoot: string,
  options: SystemsCommandOptions
): Promise<void> {
  ...
  for (const systemId of allSystems) {
    ...
    let name: string = systemId;
    let caps = "";
    let configured = chalk.dim("N/A");

    if (available) {
      const adapter = getAdapter(systemId as SystemId);
      name = adapter.name;
      ...
      const capabilities = [];
      if (adapter.capabilities.skills) capabilities.push("skills");
      if (adapter.capabilities.agents) capabilities.push("agents");
      if (adapter.capabilities.commands) capabilities.push("commands");
      if (adapter.capabilities.hooks) capabilities.push("hooks");
      if (adapter.capabilities.rules) capabilities.push("rules");
      if (adapter.capabilities.mcpServers) capabilities.push("mcp");
